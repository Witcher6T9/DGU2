...
interface DashboardProps {
  ...
  onOpenUserChat?: (channelId?: string, prefillMsg?: string, lineRef?: string) => void;
}
                    ...
                    <div className="flex items-center justify-between text-sky-200">
                      ...
                      <div className="flex items-center gap-2">
                        {onOpenUserChat && (
                          <button
                            type="button"
                            onClick={() => onOpenUserChat(`line-${activeLine.lineNo}`, undefined, `Line ${activeLine.lineNo}`)}
                            className="text-teal-300 hover:text-white font-bold text-[10px] flex items-center gap-0.5 cursor-pointer"
                            title={`Chat with Line ${activeLine.lineNo} team`}
                          >
            ...
            {layout.showQuickActions && (
              <div className="pt-3 sm:pt-4 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs relative z-10">
                <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 no-scrollbar -mx-1 px-1 sm:mx-0 sm:px-0 flex-nowrap sm:flex-wrap">
                  ...
                  {onOpenUserChat && (
                    <button
                      id="dashboard-quick-floor-chat-btn"
                      type="button"
                      onClick={() => onOpenUserChat('floor-general')}
                      className="flex items-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl bg-teal-500/20 hover:bg-teal-500/30 text-teal-200 border border-teal-400/30 font-bold transition-colors cursor-pointer shrink-0 text-xs"
                      title="Open Live Shop Floor Team Chat"
                    >
                      <MessageSquare className="w-3.5 h-3.5 text-teal-300" />
                      <span>Floor Team Chat</span>
                    </button>
                  )}
                </div>
