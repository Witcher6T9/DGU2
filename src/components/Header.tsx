      ...
      <div className="mx-auto max-w-[1500px] px-3 sm:px-6">
        <div className="flex h-14 items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Real-time Shop Floor Team Chat Button */}
            {onOpenUserChat && (
              <button
                id="top-floor-chat-btn"
                onClick={onOpenUserChat}
                title="Open Shop Floor Live Team Chat"
                aria-label="Open Shop Floor Team Chat"
                className="relative h-9 px-2.5 sm:px-3 rounded-xl border border-[#176f78]/30 bg-teal-500/10 hover:bg-[#176f78] text-[#176f78] hover:text-white flex items-center gap-1.5 transition-all text-xs font-bold cursor-pointer shadow-2xs group"
              >
                <div className="relative">
                  <MessageSquare className="w-4 h-4 shrink-0" />
                  <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 animate-pulse border border-white" />
                </div>
                <span className="hidden sm:inline font-display uppercase tracking-wide">Floor Chat</span>
                {userChatUnreadCount > 0 && (
                  <span className="px-1.5 py-0.2 rounded-full bg-rose-500 text-white text-[9px] font-bold shadow-xs">
                    {userChatUnreadCount}
                  </span>
                )}
              </button>
