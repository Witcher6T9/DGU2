...
export const DailyProductivityInsights: React.FC<DailyProductivityInsightsProps> = ({
  ...
  // Floor slider container reference and scroll states
  const floorScrollRef = useRef<HTMLDivElement>(null);
  const [canScrollFloorLeft, setCanScrollFloorLeft] = useState(false);
  const [canScrollFloorRight, setCanScrollFloorRight] = useState(false);
  ...
  const floorList = useMemo(() => ['all', ...availableFloors], [availableFloors]);
  const currentFloorIndex = Math.max(0, floorList.indexOf(selectedFloor));

  // Per-floor metrics for the slider badges
  const floorMetricsMap = useMemo(() => {
    const map = new Map<string, { count: number; hasGap: boolean; gapCount: number; target: number; achieved: number }>();
    lines.forEach(l => {
      const fl = l.floor || 'Floor 01';
      if (!map.has(fl)) {
        map.set(fl, { count: 0, hasGap: false, gapCount: 0, target: 0, achieved: 0 });
      }
      const data = map.get(fl)!;
      data.count += 1;
      data.target += l.targetProd;
      data.achieved += l.achievedProd;
      if (l.achievedProd < l.targetProd) {
        data.hasGap = true;
        data.gapCount += 1;
      }
    });
  ...
  // Floor slider scroll tracking
  const updateFloorScrollState = useCallback(() => {
    const el = floorScrollRef.current;
    if (!el) return;
    setCanScrollFloorLeft(el.scrollLeft > 6);
    setCanScrollFloorRight(el.scrollLeft < el.scrollWidth - el.clientWidth - 6);
  }, []);

  useEffect(() => {
    const el = floorScrollRef.current;
    if (!el) return;
    updateFloorScrollState();
    el.addEventListener('scroll', updateFloorScrollState, { passive: true });
    window.addEventListener('resize', updateFloorScrollState);
    return () => {
      el.removeEventListener('scroll', updateFloorScrollState);
      window.removeEventListener('resize', updateFloorScrollState);
    };
  }, [updateFloorScrollState, availableFloors]);

  const handleScrollFloorLeft = () => {
    if (floorScrollRef.current) {
      floorScrollRef.current.scrollBy({ left: -200, behavior: 'smooth' });
    }
  };

  const handleScrollFloorRight = () => {
    if (floorScrollRef.current) {
      floorScrollRef.current.scrollBy({ left: 200, behavior: 'smooth' });
    }
  };
  ...
  const handleSelectFloor = (floor: string) => {
    ...
    const chip = document.getElementById(`floor-slider-item-${floor}`);
    if (chip && floorScrollRef.current) {
      chip.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }
  };

  const handleFloorSliderRangeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const idx = parseInt(e.target.value, 10);
    if (idx >= 0 && idx < floorList.length) {
      handleSelectFloor(floorList[idx]);
    }
  };
  ...
  return (
    <section className="rounded-2xl border border-[#d9d2c2] bg-[#fbfaf6] p-5 sm:p-6 shadow-xs space-y-6">
      ...
      <div className="space-y-3.5">
        {/* Section Header & Responsive Toolbar with Dedicated Floor Slider */}
        <div className="flex flex-col gap-3.5 bg-[#f3f0e8]/90 p-3 sm:p-4 rounded-2xl border border-[#e2dcd0] shadow-2xs">
          {/* Row 1: Section Title, Subtitle, and View Switcher */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#e2dcd0]/80 pb-3">
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-[#dceceb] text-[#176f78] font-mono-numbers">
                  {filteredLines.length} {filteredLines.length === 1 ? 'Line' : 'Lines'}
                </span>
              </div>
              <p className="text-[11px] text-[#527078]">
                Real-time variance by line. Filter by floor using the interactive floor slider below.
              </p>
            </div>

            {/* View Mode & Card Navigation Controls */}
            <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
              {/* View Mode: Scroll vs Grid */}
              <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-[#d9d2c2]">
                <button
                  type="button"
                  onClick={() => setViewMode('scroll')}
                  className={`px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all min-h-[36px] cursor-pointer ${
                    viewMode === 'scroll'
              ...
              {viewMode === 'scroll' && filteredLines.length > 0 && (
                <div className="flex items-center gap-1 bg-white px-2 py-1 rounded-xl border border-[#d9d2c2] min-h-[36px]">
          ...
          {/* Row 2: Dedicated Interactive Floor Slider Track */}
          {availableFloors.length > 0 && (
            <div className="space-y-2 bg-white/70 p-2.5 sm:p-3 rounded-xl border border-[#d9d2c2]">
              {/* Floor Slider Header with active floor readout and sliding buttons */}
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-xs text-[#17343a] font-bold">
                  <Building2 className="w-3.5 h-3.5 text-[#176f78]" />
                  <span>Production Floor Slider:</span>
                  <span className="text-[#176f78] font-black bg-[#dceceb] px-2 py-0.5 rounded text-[11px]">
                    {selectedFloor === 'all' ? 'All Floors' : selectedFloor}
                  </span>
                  <span className="text-[10px] text-[#527078] font-normal hidden sm:inline">
                    ({floorList.length - 1} Unit Floors Available)
                  </span>
                </div>

                {/* Mobile / Desktop Slider Nav Arrows */}
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={handleScrollFloorLeft}
                    disabled={!canScrollFloorLeft}
                    className={`p-1.5 rounded-lg border transition-all cursor-pointer ${
                      canScrollFloorLeft
                        ? 'bg-white border-[#d9d2c2] text-[#17343a] hover:bg-[#f1eee6] shadow-2xs'
                        : 'bg-stone-50 border-stone-200 text-stone-300 cursor-not-allowed'
                    }`}
                    title="Slide floors left"
                    aria-label="Slide floors left"
                  >
                    <ChevronLeft className="w-3.5 h-3.5" />
                  </button>
                  <button
                    type="button"
                    onClick={handleScrollFloorRight}
                    disabled={!canScrollFloorRight}
                    className={`p-1.5 rounded-lg border transition-all cursor-pointer ${
                      canScrollFloorRight
                        ? 'bg-white border-[#d9d2c2] text-[#17343a] hover:bg-[#f1eee6] shadow-2xs'
                        : 'bg-stone-50 border-stone-200 text-stone-300 cursor-not-allowed'
                    }`}
                    title="Slide floors right"
                    aria-label="Slide floors right"
                  >
              ...
              {/* Horizontal Scrollable Floor Slider Track */}
              <div className="relative">
                {/* Visual fading edge indicators for overflow */}
                {canScrollFloorLeft && (
                ...
                >
                  {/* "All Floors" Slide Tile */}
                  <button
                    id="floor-slider-item-all"
                    type="button"
                    onClick={() => handleSelectFloor('all')}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all shrink-0 snap-start min-h-[44px] cursor-pointer ${
                      selectedFloor === 'all'
                        ? 'bg-[#176f78] text-white shadow-xs ring-2 ring-[#176f78]/30'
                        : 'bg-white text-[#527078] hover:text-[#17343a] border border-[#d9d2c2] hover:border-[#176f78]/40'
                    }`}
                  >
                    <span className="truncate">All Floors</span>
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded font-mono-numbers ${
                        selectedFloor === 'all'
                  ...
                  {/* Each individual production floor slide tile */}
                  {availableFloors.map(floor => {
                    const isSelected = selectedFloor === floor;
                    const metrics = floorMetricsMap.get(floor);
                    const lineCount = metrics?.count || 0;
                    const hasGap = metrics?.hasGap || false;
                    return (
                      <button
                        key={floor}
                        id={`floor-slider-item-${floor}`}
                        type="button"
              ...
              {/* Tactile Range Slider for Thumb-Scrubbing on Mobile */}
              {floorList.length > 2 && (
                <div className="pt-1 flex items-center gap-3">
                  <span className="text-[10px] text-[#527078] uppercase font-bold tracking-wider shrink-0">
                    Floor Slider:
                  </span>
                  <div className="relative flex-1 flex items-center">
                    <input
                      type="range"
                      min={0}
                      max={floorList.length - 1}
                      step={1}
                      value={currentFloorIndex}
                      onChange={handleFloorSliderRangeChange}
                      aria-label="Floor selection slider"
                      className="w-full h-1.5 bg-[#d9d2c2] rounded-lg appearance-none cursor-pointer accent-[#176f78]"
                    />
                  </div>
                  <span className="text-[10px] font-mono-numbers font-bold text-[#176f78] shrink-0">
                    {currentFloorIndex === 0 ? 'All' : `F${currentFloorIndex}`}/{floorList.length - 1}
                  </span>
                </div>
              )}
            </div>
          ...
          {/* Row 3: Gap Status Filter Pills (Optimized Touch Targets for Mobile) */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 border-t border-[#e2dcd0]/60">
            <div className="text-[11px] text-[#527078] font-medium flex items-center gap-1.5">
              <Filter className="w-3.5 h-3.5 text-[#176f78]" />
              <span>Status Filter:</span>
            </div>

            <div className="grid grid-cols-3 sm:flex items-center gap-1.5 bg-white p-1 rounded-xl border border-[#d9d2c2] text-xs">
              <button
                type="button"
                onClick={() => setFilterMode('all')}
                className={`px-3 py-1.5 rounded-lg font-bold text-[11px] transition-colors min-h-[38px] flex items-center justify-center cursor-pointer ${
                  filterMode === 'all'
                    ? 'bg-[#17343a] text-white shadow-2xs'
                    : 'text-[#527078] hover:text-[#17343a]'
                }`}
              >
                All ({lines.filter(l => selectedFloor === 'all' || l.floor === selectedFloor).length})
              </button>
              <button
                type="button"
                onClick={() => setFilterMode('gap')}
                className={`px-3 py-1.5 rounded-lg font-bold text-[11px] transition-colors flex items-center justify-center gap-1.5 min-h-[38px] cursor-pointer ${
                  filterMode === 'gap'
