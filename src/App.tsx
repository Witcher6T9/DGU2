...
import { FloatingChatCluster } from './components/FloatingChatCluster';
...
export default function App() {
  ...
  const [userChatInitialChannel, setUserChatInitialChannel] = useState<string>('floor-general');
  const [userChatPrefillMsg, setUserChatPrefillMsg] = useState<string | undefined>();
  const [userChatPrefillLine, setUserChatPrefillLine] = useState<string | undefined>();

  const handleOpenUserChat = (channelId = 'floor-general', prefillMsg?: string, lineRef?: string) => {
    setUserChatInitialChannel(channelId);
    setUserChatPrefillMsg(prefillMsg);
    setUserChatPrefillLine(lineRef);
  ...
  return (
    <div className="cockpit-shell min-h-[100dvh] flex flex-col bg-[hsl(var(--background))] text-[hsl(var(--foreground))] antialiased">
      ...
      <Header
        ...
        onOpenUserChat={() => handleOpenUserChat('floor-general')}
        profile={profile}
      ...
      {/* Animated Floating Action Cluster: Shop Floor Live Chat & IE AI Assistant */}
      <FloatingChatCluster
        onOpenFloorChat={() => handleOpenUserChat('floor-general')}
        onOpenAIChat={() => setIsChatOpen(true)}
      />

      {/* Real-time Multi-Channel Shop Floor Team Chat Modal */}
      <UserChatModal
        isOpen={isUserChatOpen}
        onClose={() => setIsUserChatOpen(false)}
        profile={profile}
        lines={lines}
        initialChannelId={userChatInitialChannel}
        prefillMessage={userChatPrefillMsg}
        prefillLineRef={userChatPrefillLine}
      />
