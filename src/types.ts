...
export type ChatMessageTag = 'URGENT' | 'BOTTLENECK' | 'BREAKDOWN' | 'QUALITY' | 'HANDOVER' | 'INFO' | 'BALANCING';

export interface ChatTelemetrySnippet {
  lineNo: string;
  metricName: string;
  value: string;
...
export type ChannelCategory = 'factory' | 'lines' | 'roles' | 'direct';
...
export interface OnlineUserPresence {
  ...
  jobTitle: string;
  avatarText: string;
  status: 'online' | 'on_floor' | 'busy' | 'break';
  activeLine?: string;
  lastActive: string;
}
