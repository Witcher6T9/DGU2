...
async function startServer() {
  ...
  // --- Real-time User Chat REST Endpoints ---
  app.get('/api/user-chat/channels', (req, res) => {
    res.json({ channels: chatService.getChannels() });
  });

  app.get('/api/user-chat/messages', (req, res) => {
    const channelId = String(req.query.channelId || 'floor-general');
    const limit = req.query.limit ? parseInt(String(req.query.limit), 10) : 60;
    res.json({ messages: chatService.getMessages(channelId, limit) });
  });

  app.post('/api/user-chat/messages', (req, res) => {
    try {
      const msg = chatService.addMessage(req.body);
  ...
  app.post('/api/user-chat/react', (req, res) => {
    const { messageId, emoji, userName } = req.body;
    if (!messageId || !emoji || !userName) {
      return res.status(400).json({ error: 'messageId, emoji, and userName are required' });
    }
    const updated = chatService.toggleReaction(messageId, emoji, userName);
    if (!updated) {
  ...
  app.post('/api/user-chat/pin', (req, res) => {
    const { messageId } = req.body;
    if (!messageId) {
      return res.status(400).json({ error: 'messageId is required' });
    }
    const updated = chatService.togglePin(messageId);
    if (!updated) {
      return res.status(404).json({ error: 'Message not found' });
    }
    res.json({ success: true, message: updated });
  ...
  app.delete('/api/user-chat/messages/:id', (req, res) => {
    const deleted = chatService.deleteMessage(req.params.id, String(req.query.requesterName || ''));
    if (!deleted) {
      return res.status(404).json({ error: 'Message not found' });
    }
  ...
  app.get('/api/user-chat/online-users', (req, res) => {
    res.json({ onlineUsers: chatService.getOnlineUsers() });
  });

  app.post('/api/user-chat/presence', (req, res) => {
    chatService.setOrUpdateUserPresence(req.body);
    res.json({ success: true });
  });
...
Return ONLY valid JSON matching this exact structure:
{
  ...
  // Setup real-time WebSocket on /ws/chat
